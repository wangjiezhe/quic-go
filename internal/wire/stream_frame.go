...
func (f *StreamFrame) Write(b *bytes.Buffer, version protocol.VersionNumber) error {
	...
	streamIDLen := f.getStreamIDLength()
	...
	if version <= protocol.Version39 {
		...
		typeByte ^= uint8(streamIDLen-1) << 3
	}
...
func (f *StreamFrame) getStreamIDLength() protocol.ByteCount {
	if f.StreamID < (1 << 8) {
		return 1
...
func (f *StreamFrame) MinLength(version protocol.VersionNumber) (protocol.ByteCount, error) {
	length := protocol.ByteCount(1) + f.getStreamIDLength() + f.getOffsetLength(version)
	if f.DataLenPresent {
		length += 2
	}
	return length, nil
